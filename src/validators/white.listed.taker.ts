...
export const whiteListedTaker = (): OrderValidator => {
  return (order: OrderData, config: ExecutorConfig, context: ValidatorContext): Promise<boolean> => {
    const chainConfig = config.chains.find(chain => chain.chain === order.take.chainId)!;
    let result = false;
    if (chainConfig.chain === ChainId.Solana) {
      //todo
    } else {
      const web3 = createWeb3WithPrivateKey(chainConfig.chainRpc, chainConfig.takerPrivateKey);
      result = web3.eth.defaultAccount === chainConfig.beneficiary;
    }
    context.logger.info(`approve status: ${result}, beneficiary ${chainConfig.beneficiary}`);
