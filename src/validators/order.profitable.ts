...
export const orderProfitable = (profitabilityBps: number): OrderValidator => {
  return async (order: OrderData, config: ExecutorConfig, context: ValidatorContext): Promise<boolean> => {
    ...
    const profitability = takeUsdAmount.multipliedBy(profitabilityBps).div(100 ** 2);
    ...
    logger.info(`approve status: ${result}, giveUsdAmount: ${giveUsdAmount.toString()}, takeUsdAmount: ${takeUsdAmount.toString()}`);
