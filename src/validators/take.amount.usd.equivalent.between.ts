 ...
 *
 */
export const takeAmountUsdEquivalentBetween = (minUSDEquivalent: number, maxUSDEquivalent: number): OrderValidator => {
  return async (order: OrderData, config: ExecutorConfig, context: ValidatorContext): Promise<boolean> => {
    const logger = context.logger.child({ validator: 'takeAmountUsdEquivalentBetween' });
    let giveWeb3;
    if (order.take.chainId !== ChainId.Solana) {
      giveWeb3 = new Web3(config.fulfillableChains!.find(chainConfig => chainConfig.chain === order.take.chainId)!.chainRpc);
    }
    const giveAddress = helpers.bufferToHex(Buffer.from(order.take.tokenAddress));
