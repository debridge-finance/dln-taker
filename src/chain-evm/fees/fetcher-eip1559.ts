...
export const getEip1559FeeFetcher =
  (
  ...
  async (gasCategory: GasCategory, connection: Web3, logger?: Logger): Promise<EIP1551Fee> => {
    const blocksNum = 10;
    ...
    logger?.debug(
      `desired priority fee: ${maxPriorityFeePerGas} (prev blocks: ${blocksNum}, gasCategory: ${GasCategory[gasCategory]}, percentile: ${priorityFeePercentiles[gasCategory]}, boundary: ${priorityFeeIncreaseBoundary})`,
    );
    ...
    logger?.debug(
      `target base fee: ${targetBaseFee} (found base fee: ${baseFee}, gasCategory: ${GasCategory[gasCategory]}, multiplier: ${baseFeeMultipliers[gasCategory]})`,
    );
...
export const defaultEip1559FeeFetcher = getEip1559FeeFetcher(
  {
