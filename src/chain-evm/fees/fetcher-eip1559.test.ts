...
import { EIP1551Fee, getEip1559FeeFetcher, priorityFeeEstimator } from './fetcher-eip1559';
...
describe('EIP1559 fee fetchers', () => {
  it('eip1559FeeFetcher should correctly pick a fee', async () => {
    ...
    for (const [gasCategory, expectedFee] of expectedFees) {
      const feeFetcher = getEip1559FeeFetcher(baseFeeMultipliers, priorityFeePercentiles);
      const actualFee = await feeFetcher(gasCategory, conn, createTestFrameworkLogger());
      expect(actualFee).to.deep.eq(expectedFee, `mocked: ${GasCategory[gasCategory]}`);
    }
  });

  it('priorityFeeEstimator should correctly pick a tip', async () => {
    const rewards = [
      [100_000_000_000n.toString()],
      [105_000_000_000n.toString()],
      [102_000_000_000n.toString()],
