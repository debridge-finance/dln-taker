...
export class EvmTransactionBuilder implements TransactionBuilder {
    constructor(private readonly chain: ChainId, private contractsForApprove: string[], private connection: Web3, private readonly adapter: EvmProviderAdapter, private readonly executor: IExecutor) {}

    getOrderFulfillTxSender(orderEstimation: OrderEstimation, logger: Logger) {
      return async () => this.adapter.sendTransaction(
        await new EVMOrderFulfillIntent(orderEstimation.order, orderEstimation, logger)
      ...
      logger.debug('Collect ERC-20 tokens that should have approvals')
        ...
        for (const token of tokens) {
          for (const contract of this.contractsForApprove) {
            const currentAllowance = await getAllowance(this.connection, token, this.adapter.address, contract);
            if (currentAllowance === 0n) {
              logger.debug(`${token} requires approval`)
              const func = () => {
                logger.debug(`Setting approval on ${token} to be spend by ${contract} on behalf of ${this.adapter.address}`)
