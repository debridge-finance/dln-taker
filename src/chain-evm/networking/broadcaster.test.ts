...
import { EvmTxBroadcaster } from './broadcaster';
...
async function expectSuccessfulBroadcast(broadcaster: EvmTxBroadcaster, expectedNonce: number) {
...
describe(`Broadcaster`, function () {
  ...
  it('Must accept transaction immediately', async () => {
    ...
    const broadcaster = new EvmTxBroadcaster(
      ...
      undefined,
      hre.web3,
      new EvmFeeManager(hre.web3, true, {}, {}),
  ...
  it('Must be rejected upon sending', async () => {
    ...
    const sender = new EvmTxBroadcaster(
      this.ctx.tx,
      undefined,
      hre.web3,
      new EvmFeeManager(hre.web3, true, {}, {}),
  ...
  it('Must reject txn if capped fee is reached', async () => {
    const { logger, signer } = this.ctx;

    await hre.ethers.provider.send('evm_setAutomine', [false]);
    const sendMaxAttempts = 3;
    const cappedFee =
      BigInt(await hre.web3.eth.getGasPrice()) *
      BigInt(await hre.web3.eth.estimateGas(this.ctx.tx));
    const overcappingAllowance = 2;
    const aggressiveMultiplier = 2.00001;
    ...
    const sender = new EvmTxBroadcaster(
      ...
      new EvmFeeManager(
        ...
        true,
        {
          ...
          overcappingAllowance,
          legacyGasPriceNormalMultiplier: 1,
          legacyGasPriceAggressiveMultiplier: aggressiveMultiplier,
          replaceBumperNormalMultiplier: 1.01,
          replaceBumperAggressiveMultiplier: 1.01,
    ...
    await expect(sender.broadcastAndWait()).to.be.rejectedWith('Unable to populate pricing');
  });
});
