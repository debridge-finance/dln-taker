...
export class EVMOrderEstimator extends OrderEstimator {
    ...
    private async getEstimatedGasPrice(): Promise<bigint> {
      ...
      const estimatedGasPrice = currentGasPrice * BigInt(EVMOrderEstimator.EVM_FULFILL_GAS_PRICE_MULTIPLIER * 10_000) / 10_000n;
    ...
    protected async getExpectedTakeAmountContext(): Promise<Parameters<typeof calculateExpectedTakeAmount>['2']> {
      ...
      return {
        ...parentContext,
        evmFulfillGasLimit: this.getPayloadEntry<number>(EVMOrderValidator.EVM_FULFILL_GAS_LIMIT_NAME),
