import { ChainId, ZERO_EVM_ADDRESS, ZERO_SOLANA_ADDRESS } from "@debridge-finance/pmm-client";
...
export class CoingeckoPriceFeed implements PriceFeed {
	...
	async getPrice(chainId: ChainId, token: string): Promise<number> {
		...
		if (chainId === ChainId.Solana) {
			tokenAddress = tokenAddress.startsWith("0x") ? new PublicKey(helpers.hexToBuffer(tokenAddress)).toBase58() : tokenAddress;
