 ...
 */
export function whitelistedReceiver(addresses: string[]): OrderFilterInitializer {
  return async (
    chainId: ChainId,
    /* context: OrderFilterInitContext */ {},
  ): Promise<OrderFilter> => {
    const addressesBuffer = addresses.map((address) => tokenStringToBuffer(chainId, address));
    return async (order: OrderData, context: FilterContext): Promise<boolean> => {
      const logger = context.logger.child({ filter: 'WhiteListedReceiver' });
      const result = addressesBuffer.some((address) => buffersAreEqual(order.receiver, address));
