...
export const orderRejected = (
    ...
    return async (arg: HookParams<Hooks.OrderRejected>) => {
        ...
        const message = `Order #<a href="https://dln.debridge.finance/order?orderId=${arg.order.orderId}">${arg.order.orderId}</a>${attempts} (${giveInfo} -> ${takeInfo}) has been rejected because of ${ RejectionReason[arg.reason] }: ${ arg.message }`
