...
export class DataStore {
  ...
  private async getPendingForUnlockOrderIds(from: ChainId): Promise<string[]> {
    ...
    while (hasMoreOrders) {
      ...
      orderIds.push(...getForUnlockOrders.orders.map((orderDTO) => orderDTO.orderId.stringValue));
  ...
  private static convertOrder(
    order: Awaited<ReturnType<StatsAPI['getOrderLiteModel']>>,
  ): OrderDataWithId {
    return {
      ...
      orderAuthorityDstAddress: DataStore.parseBytesArray(
        order.orderAuthorityAddressDst.bytesArrayValue,
      ),
      allowedTaker: order.allowedTakerDst.bytesArrayValue
        ? DataStore.parseBytesArray(order.allowedTakerDst.bytesArrayValue)
        : undefined,
      allowedCancelBeneficiary: order.allowedCancelBeneficiarySrc.bytesArrayValue
        ? DataStore.parseBytesArray(order.allowedCancelBeneficiarySrc.bytesArrayValue)
        : undefined,
