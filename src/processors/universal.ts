...
class UniversalProcessor extends BaseOrderProcessor {
  ...
  private async tryProcess(
    orderInfo: IncomingOrder<
      OrderInfoStatus.Created | OrderInfoStatus.ArchivalCreated
    >,
  ...
  private async processOrder(
    ...
    if (orderInfo.status === OrderInfoStatus.Created) {
      const finalizationInfo = (
        orderInfo as IncomingOrder<OrderInfoStatus.Created>
      ).finalization_info;
      if (finalizationInfo === "Revoked") {
        ...
        const announcedConfirmation =
          finalizationInfo.Confirmed.confirmation_blocks_count;
        logger.info(
          `order announced with custom finality, announced confirmation: ${announcedConfirmation}`
        );
        ...
        const [giveTokenUsdRate, giveTokenDecimals] = await Promise.all([
          context.config.tokenPriceService.getPrice(
            ...
            buffersAreEqual(
              orderInfo.order.give.tokenAddress,
              tokenStringToBuffer(ChainId.Ethereum, ZERO_EVM_ADDRESS)
            )
              ? null
              : orderInfo.order.give.tokenAddress,
            {
          ...
          context.config.client.getDecimals(
            orderInfo.order.give.chainId,
            orderInfo.order.give.tokenAddress,
            context.giveChain.fulfullProvider.connection as Web3
          ),
        ]);
        ...
        const range = context.config.chains[
          orderInfo.order.give.chainId
        ]!.usdAmountConfirmations.find(
          (usdWorthRange) =>
            usdWorthRange.usdWorthFrom < usdWorth &&
            usdWorth <= usdWorthRange.usdWorthTo
        );

        // range found, ensure current block confirmation >= expected
        if (range?.minBlockConfirmations) {
          logger.debug(
            `usdAmountConfirmationRange found: (${range.usdWorthFrom}, ${range.usdWorthTo}]`
          );

          if (announcedConfirmation < range.minBlockConfirmations) {
            logger.info(
              "announced block confirmations is less than the block confirmation constraint; skipping the order"
            );
            return;
          } else {
          ...
          logger.debug(
            "non-finalized order is not covered by any custom block confirmation range, skipping"
          );
          return;
        }
    ...
    await this.waitIsOrderFulfilled(
      orderInfo.orderId,
      orderInfo.order,
      context,
      logger
