...
export class NonFinalizedOrdersBudgetController {
  ...
  isFitsBudget(orderId: string, usdValue: number): boolean {
    ...
    if (potentialSpentBudgetInUSD > this.budget) {
      this.#logger
        .child({ orderId })
        .debug(
          `order worth $${usdValue} does not fit budget; new budget utilization: ${potentialSpentBudgetInUSD} (${this.getBudgetUtilizationRate(
            potentialSpentBudgetInUSD,
          )}%)`,
        );
      return false;
    }
  ...
  addOrder(orderId: string, usdValue: number): void {
    ...
    this.#logger
      .child({ orderId })
      .debug(
        `order worth $${usdValue} has been added, new budget utilization: ${
          this.#spent
        } (${this.getBudgetUtilizationRate()}%)`,
      );
  }
  ...
  removeOrder(orderId: string) {
    ...
    this.#logger
      .child({ orderId })
      .debug(
        `order worth $${usdValue} has been removed, new budget utilization: ${
          this.#spent
        } (${this.getBudgetUtilizationRate()}%)`,
      );
  }
