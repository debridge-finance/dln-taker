...
export async function tgNotify(text: string): Promise<any> {
  return axios.post(`https://api.telegram.org/bot${process.env.TG_BOT}/sendMessage`, {
...
export async function tgNotifyOrder(id: string, order: OrderData, text: string): Promise<any> {
  return tgNotify(
    `order ${id} (${ChainId[order.give.chainId]} -> ${
      ChainId[order.take.chainId]
    }, receiver ${tokenAddressToString(order.take.chainId, order.receiver)}) ${text}`,
  );
}
