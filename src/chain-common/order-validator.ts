...
export enum OrderValidationResult {
    Successful, ShouldPostpone, ShouldReject
}
export type OrderValidation<T extends OrderValidationResult> = { result: T;  }
& (T extends OrderValidationResult.Successful ? { estimator: OrderEstimator; payload: OrderEvaluationPayload  } : { })
& (T extends OrderValidationResult.ShouldReject ? { rejection: RejectionReason
    message: string; } : {})
& (T extends OrderValidationResult.ShouldPostpone ? { postpone: PostponingReason;
    message: string;
    delay?: number} : {})
...
export class OrderValidator extends OrderEvaluationContextual {
    ...
    constructor(protected readonly order: CreatedOrder, context: { logger: Logger }) {
        super()
    ...
    protected getOrderEstimator(): OrderEstimator {
        return new OrderEstimator(this.order, { logger: this.#logger, preferEstimation: this.#preliminarySwapResult, validationPayload: this.payload })
    }

    private async checkAllowedTaker(): Promise<void> {
        if (this.order.orderData.allowedTaker) {
            if (!buffersAreEqual(this.order.takeChain.unlockProvider.bytesAddress, this.order.orderData.allowedTaker)
            ) {
    ...
    private async checkFulfillmentDelay(): Promise<void> {
        if (this.order.status !== OrderInfoStatus.Created) return;

        const [srcConstraints, dstConstraints] = await Promise.all(
            [this.order.srcConstraints(), this.order.dstConstraints()]
        )

        // determine if we should postpone the order
        const fulfillmentDelay = dstConstraints.fulfillmentDelay || srcConstraints.fulfillmentDelay || 0;
        const remainingDelay = getOrderRemainingDelay(
            this.order.arrivedAt,
            fulfillmentDelay,
        );
    ...
    private async checkTVLBudget(): Promise<void> {
        ...
        if (TVLBudgetController.budget > 0) {
            ...
            if (currentGiveTVL + usdValue > TVLBudgetController.budget) {
                ...
                } over a budget of $${TVLBudgetController.budget} (current TVL: $${currentGiveTVL}), thus postponing`;
    ...
    private async checkFinalization(): Promise<void> {
        if (this.order.finalization === 'Finalized') {
            ...
            if (srcConstraints) {
                this.#logger.debug(
                    `usdAmountConfirmationRange found: <=$${srcConstraints.upperThreshold}`,
                );

                if (confirmationBlocksCount < srcConstraints.minBlockConfirmations) {
                    const message = `announced block confirmations is less than the block confirmation constraint (${srcConstraints.minBlockConfirmations
                        } for order worth of $${usdValue}`;
                    return rejectOrder(
    ...
    private async checkPrefulfillSwapAbility(): Promise<void> {
        ...
        if (
            ...
            const message = `swaps are unavailable on ${ChainId[take.chainId]
                }, can't perform pre-fulfill swap from ${pickedBucket.reserveDstToken.toAddress(take.chainId)} to ${take.tokenAddress.toAddress(take.chainId)
        }`;
    ...
    private async checkAccountBalance(): Promise<void> {
        ...
        if (accountReserveBalance < maxProfitableReserveAmount) {
            const message =
                `not enough funds of the reserve token (${route.reserveDstToken.toAddress(take.chainId)}); actual balance: ${await this.order.executor.formatTokenValue(
                    take.chainId,
                    route.reserveDstToken,
    ...
    private async checkRoughProfitability(): Promise<void> {
        ...
        if (this.order.route.requiresSwap) {
            ...
            if (this.#preliminarySwapResult.amountOut < this.order.orderData.take.amount) {
                return postponeOrder(PostponingReason.NOT_PROFITABLE, 'rough profitability estimation: order is not profitable')
            }
        }
        else {
            if (maxProfitableReserveAmount < this.order.orderData.take.amount) {
                return postponeOrder(PostponingReason.NOT_PROFITABLE, 'rough profitability estimation: order is not profitable')
            }
        }
    }
